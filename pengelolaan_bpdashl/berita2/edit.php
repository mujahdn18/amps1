    ...
    <div class="box">
        <h1>Berita</h1>
            ...
            <div class="row">
                ...
                <?php
                $id = @$_GET['id'];
                $sql_berita = mysqli_query($con, "SELECT * FROM tb_berita WHERE id_berita = '$id'") or die (mysqi_error($con));
                $data = mysqli_fetch_array($sql_berita);
                ?>
                    <form action="proses.php" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="judul_berita">Judul Berita</label>
                            <input type="hidden" name="id" value="<?=$data['id_berita']?>">
                            <textarea name="judul_berita" id="judul_berita" class="form-control" required autofocus><?=$data['judul_berita']?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="tgl_berita">Tanggal</label>
                            <input type="date" name="tgl_berita" id="tgl_berita" value="<?=$data['tgl_berita']?>" class="form-control" required>
                        </div> 
                        <div class="form-group">
