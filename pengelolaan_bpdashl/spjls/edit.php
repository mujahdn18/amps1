...
?>
    ...
    <div class="box">
        <h1>SPJ LS</h1>
            <h4>
                <small>Edit Data SPJ Belanja Langsung</small>
            ...
            <div class="row">
                ...
                <?php
                $id = @$_GET['id'];
                $sql_spjls = mysqli_query($con, "SELECT * FROM tb_spjls WHERE id_spjls = '$id'") or die (mysqi_error($con));
                $data = mysqli_fetch_array($sql_spjls);
                ?>    
                    <form action="proses.php" method="post">
                    <div class="form-group">
                            <label for="id_spt">No SPT</label>
                            <input type="hidden" name="id" value="<?=$data['id_spjls']?>">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="no_spm" id="no_spm" value="<?=$data['no_spm']?>" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label for="tgl_spm">Tanggal SPM</label>
                            <input type="date" name="tgl_spm" id="tgl_spm" value="<?=$data['tgl_spm']?>" class="form-control" required>
                        </div>  
                        <div class="form-group">
