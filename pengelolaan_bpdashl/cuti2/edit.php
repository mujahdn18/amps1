...
?>
    ...
    <div class="box">
        <h1>Surat Cuti</h1>
            <h4>
                <small>Edit Data Surat Cuti</small>
            ...
            <div class="row">
                ...
                <?php
                $id = @$_GET['id'];
                $sql_cuti = mysqli_query($con, "SELECT * FROM tb_cuti WHERE id_cuti = '$id'") or die (mysqi_error($con));
                $data = mysqli_fetch_array($sql_cuti);
                ?>
                    <form action="proses.php" method="post">
                        <div class="form-group">
                            <input type="hidden" name="id" value="<?=$data['id_cuti']?>">
                            <label for="tgl_cuti">Tanggal Cuti</label>
                            <input type="date" name="tgl_cuti" id="tgl_cuti" value="<?=$data['tgl_cuti']?>" class="form-control" required autofocus>
                        </div> 
                        <div class="form-group">
                            <label for="perihal">Perihal</label>
                            <select name="perihal" id="perihal" class="form-control" required>
                                <option value="<?=$data['perihal']?>"><?=$data['perihal']?></option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="jangka_hari" id="jangka_hari" value="<?=$data['jangka_hari']?>" class="form-control" required >
                        </div> 
                        <div class="form-group">
